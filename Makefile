...
dev:    ## Run the server in dev mode
	poetry run uvicorn python_testing.main:app --reload --host 0.0.0.0 --port 8000
...
.PHONY: check-format
check-format:
	poetry run yapf --diff --recursive python_testing/**/*.py

.PHONY: check-style
check-style:
	poetry run flake8 python_testing/
	poetry run pylint python_testing/**

.PHONY: reformat
reformat:  ## Format python code
	poetry run yapf --parallel --recursive -ir python_testing/
